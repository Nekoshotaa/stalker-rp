...
ITEM.description = 'Сладкий и калорийный перекус. Быстро поднимает силы, когда времени на нормальную еду нет.'
ITEM.model = 'models/kek1ch/dev_chocolate.mdl'

ITEM.uniqueID = 'food_chocolate_bar'
ITEM.price = 210
ITEM.width = 1
ITEM.height = 1
ITEM.hunger = 9
...
ITEM.functions.Use = {
    ...
    OnRun = function(item)
        ...
        if client.AddBuff then client:AddBuff("buff_staminarestore", 10, { amount = 0.80 }) end
        ix.chat.Send(client, "iteminternal", "съедает "..item.name..".", false)
