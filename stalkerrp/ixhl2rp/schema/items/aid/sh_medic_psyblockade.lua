...
ITEM.description = "Сильный препарат, подавляющий навязчивые психические эффекты. Обычно используют перед походом в опасные аномальные районы."
ITEM.model = "models/wick/wrbstalker/cop/newmodels/items/wick_psy_blockade.mdl"
ITEM.price = 2100
ITEM.width = 1
ITEM.height = 1
...
function ITEM:PopulateTooltipIndividual(tooltip)
    ix.util.PropertyDesc(tooltip, "Психическая защита", Color(160, 120, 220))
end
...
ITEM.functions.Use = {
    ...
    OnRun = function(item)
        ...
        if client.AddBuff then
            client:AddBuff("buff_psysuppress", 120, {})
            client:AddBuff("buff_psyblock", 120, { amount = 0.15 })
        end
