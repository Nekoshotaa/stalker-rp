...
if CLIENT then
	...
	function PLUGIN:SendTradeToVendor(itemObject, isSellingToVendor)
		...
		local entity = IsValid(ix.gui.vendorRemake) and ix.gui.vendorRemake.entity or nil
		...
		local vendorPanel = ix.gui.vendorRemake
		if (IsValid(vendorPanel) and vendorPanel.SetSelectedItem) then
			vendorPanel:SetSelectedItem(itemObject.uniqueID)
		end
		...
		if curInv and newInventory then
			if (newInventory.vars and newInventory.vars.isNewVendor and curInv.slots) or (curInv.vars and curInv.vars.isNewVendor and newInventory.slots) then
				...
				if IsValid(ix.gui.vendorRemake) and not IsValid(ix.gui.vendorRemakeEditor) then -- sell / purchase that item
					...
					if (not IsValid(entity) or not entity.items[itemObject.uniqueID]) then
						return
					end
...
if (SERVER) then	
		...
		for _, v in ipairs(self:GetData() or {}) do
			...
			entity:BuildInventory(function(inventory)
				for uniqueID, data in pairs(v.items) do
					...
					if (mode == VENDOR.SELLANDBUY or mode == VENDOR.SELLONLY) then
	...
	net.Receive("ixVendorRemakeEdit", function(len, client)
		...
		elseif (key == 'inventory_size') then
			...
			timer.Create("ixVendorRemakeRestoreInvSize", 1, 1, function()
				entity:BuildInventory(function(inventory)
					...
					UpdateEditReceivers(entity.receivers, key, {invW, invH})
				end, invW, invH)
			end)
		...
		elseif (key == "category") then
			...
			entity.items[uniqueID].category = categoryName
			data = {uniqueID, categoryName}
		...
		elseif (key == "mode") then
			...
			local shouldExist = (mode == VENDOR.SELLANDBUY or mode == VENDOR.SELLONLY)

			if (shouldExist and #items == 0 and !inventory:Add(uniqueID, 1, nil, nil, nil, true)) then
				feedback = false
			else
				if (!shouldExist and #items > 0) then
	...
	local function GetVendorContext()
		local panel = ix.gui.vendorRemake
		if (IsValid(panel) and IsValid(panel.entity)) then
			return panel.entity, panel
		end

		local editor = ix.gui.vendorRemakeEditor
		if (IsValid(editor) and IsValid(editor.entity)) then
			return editor.entity, editor
		end
	end

	local function ReadVendorMoney()
	...
	local function ApplyEditorLine(editor, uniqueID)
		if (not IsValid(editor) or not IsValid(editor.entity) or not editor.lines or not editor.lines[uniqueID]) then
			return
		end

		local mode = editor.entity.items[uniqueID] and editor.entity.items[uniqueID][VENDOR.MODE]
		local current, max = editor.entity:GetStock(uniqueID)

		editor.lines[uniqueID]:SetValue(2, mode and L(VENDOR_TEXT[mode]) or L"none")
		editor.lines[uniqueID]:SetValue(3, editor.entity:GetPrice(uniqueID))
		editor.lines[uniqueID]:SetValue(4, max and (current .. "/" .. max) or "-")
	end
	...
	function PLUGIN:CreateItemInteractionMenu(item_panel, menu, itemTable)
		...
		local entity = select(1, GetVendorContext())
		if (not IsValid(entity)) then
			return
		end
		...
		if inventory.vars.isNewVendor then
			if (data == VENDOR.SELLANDBUY or data == VENDOR.SELLONLY) then
				menu:AddOption(L"purchase", function()
					surface.PlaySound("stalkersound/items/inv_items_wpn_1.wav")
			...
			if (data == VENDOR.SELLANDBUY or data == VENDOR.BUYONLY) then
				menu:AddOption(L"sell", function()
					surface.PlaySound("stalkersound/inv_ruck.wav")
	...
	net.Receive("ixVendorRemakeEdit", function()
		...
		elseif (key == "category") then
			local uniqueID = data[1]
			entity.items[uniqueID] = entity.items[uniqueID] or {}
			entity.items[uniqueID].category = data[2]
		end
	end)
	...
	net.Receive("ixVendorRemakeEditFinish", function()
		...
		if (!IsValid(editor) or !IsValid(entity)) then
			return
		end
		...
		elseif (key == "mode") then
			ApplyEditorLine(editor, data[1])
			editor:RefreshAllItems()
		elseif (key == "category") then
			local uniqueID = data[1]
			entity.items[uniqueID] = entity.items[uniqueID] or {}
			entity.items[uniqueID].category = data[2]
		...
		elseif (key == "remove_inv_item") then
			if (editor.lines[data[2]]) then
				editor.lines[data[2]]:SetValue(2, L"none")
			end
			editor:RefreshAllItems()
		elseif (key == "money") then
			editor.money:SetText(tostring(entity.money or 0))
		end

		if (IsValid(ix.gui.vendorRemake) and ix.gui.vendorRemake.RefreshCatalog) then
			ix.gui.vendorRemake:RefreshCatalog()
		end
	...
	net.Receive("ixVendorRemakeEditor", function()
		...
		if (inventory and inventory.slots) then
			...
			ix.gui.vendorRemakeEditor.entity = entity
			ix.gui.vendorRemakeEditor:Setup()
		end
	end)
	...
	net.Receive("ixVendorRemakeMoney", function()
		...
		if (IsValid(ix.gui.vendorRemake) and ix.gui.vendorRemake.UpdateInfoText) then
			ix.gui.vendorRemake:UpdateInfoText()
		end
	end)
	...
	net.Receive("ixVendorRemakeStock", function()
		...
		if (IsValid(editor) and editor.lines[uniqueID]) then
...
properties.Add("vendor_remake_edit", {
	...
	Receive = function(self, length, client)
		...
		net.Start("ixVendorRemakeEditor")
			net.WriteEntity(entity)
			net.WriteBool(entity.money != nil)
			if (entity.money != nil) then
				net.WriteUInt(math.max(entity.money, 0), 32)
			end
