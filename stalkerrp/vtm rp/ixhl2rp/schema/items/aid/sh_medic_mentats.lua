ITEM.name = 'Таблетки ясности'
ITEM.description = 'Маленькая коробочка аптекарских таблеток. На время прогоняет туман из головы и помогает держаться собранно, но после действия оставляет сухость во рту.'
ITEM.model = 'models/fnv/clutter/chems/mentats01.mdl'
ITEM.price = 520
ITEM.width = 1
ITEM.height = 1
...
function ITEM:PopulateTooltipIndividual(tooltip)
    ix.util.PropertyDesc(tooltip, "Стимулятор ясности", Color(180, 180, 240))
end
...
ITEM.functions.Use = {
    ...
    OnRun = function(item)
        ...
        if client.AddBuff then
            client:AddBuff("buff_staminarestore", 30, { amount = 1 })
        end
        clampNeeds(client, -2, -5)
