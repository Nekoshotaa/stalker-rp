ITEM.name = 'Шприц доктора'
ITEM.description = 'Дорогой медицинский шприц с сильным восстановительным составом. Не спрашивай, что именно смешал доктор, если после укола ты всё ещё можешь стоять.'
ITEM.model = 'models/fnv/clutter/health/stimpak01.mdl'
ITEM.price = 850
ITEM.width = 1
ITEM.height = 1
...
function ITEM:PopulateTooltipIndividual(tooltip)
    ix.util.PropertyDesc(tooltip, "Сильное лечение", Color(120, 240, 120))
end
...
ITEM.functions.Use = {
    ...
    OnRun = function(item)
        ...
        if client.AddBuff then
            client:AddBuff("buff_rapidheal", 8, { amount = 1 })
        end
        clampNeeds(client, -2, -2)
