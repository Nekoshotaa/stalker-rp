ITEM.name = "Грибной суп «Смельчак»"
ITEM.description = "Редкий густой суп из обычных грибов и мухоморов. Вкус у него странный, но опытные сталкеры уверяют, что после такой еды Зона будто начинает шептать чуть громче."
ITEM.model = "models/props/stalcraft/kastrulyacheesesoup.mdl"
ITEM.uniqueID = "food_mushroom_soup_smelchak"
ITEM.category = "Еда"
...
function ITEM:GetDescription()
    ...
    desc = desc .. "\nДаёт постоянный бонус: +1 к Чутью Зоны"
    return desc
end

function ITEM:PopulateTooltipIndividual(tooltip)
    ix.util.PropertyDesc(tooltip, "Особая еда", Color(205, 170, 125))
    ix.util.PropertyDesc(tooltip, "+1 Чутьё Зоны", Color(120, 200, 120))
end
...
ITEM.functions.Use = {
    ...
    OnRun = function(item)
        ...
        if #changes > 0 then
            client:Notify("Мир вокруг будто стал яснее: " .. table.concat(changes, ", ") .. ".")
        end
