...
if (SERVER) then
	...
	function ENT:Use(activator)
		...
		local inventory = self:GetInventory()
		if (!inventory) then return end
		if ((activator.ixNextOpen or 0) >= CurTime()) then return end
		...
		if (def and def.isTouristLocked == true and activator.getReputation and activator:getReputation() < 25 and !activator:IsAdmin()) then
		...
		if (self:GetLocked() and !self.Sessions[character:GetID()]) then
			self:EmitSound((def and def.locksound) or "doors/default_locked.wav")
	...
	function ENT:OnPopulateEntityInfo(tooltip)
		...
		title:SetImportant()
		title:SetText(self:GetDisplayName() != "" and self:GetDisplayName() or "Container")
		...
		description:SetText((definition and definition.description) or "A storage container.")
